...
export default function Home() {
  ...
  const submitDisabled = submitting || !isConnected;
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!intentInput.trim() || !isConnected) return;
        ...
        <header className="flex flex-col gap-10">
          <nav className="nav-shell">
            <div className="nav-brand">
              <span className="nav-logo">Δ</span>
              <div>
                <p className="nav-label">De-concierge</p>
                <p className="nav-subtitle">Verifiable booking intelligence for humans & agents</p>
              </div>
            </div>
            <div className="nav-links">
              <span className="nav-link">Story</span>
              <span className="nav-link">Proofs</span>
              <span className="nav-link">Roadmap</span>
            </div>
            <div className="nav-actions">
              <span className="nav-pill">Syncing calendars since 2025</span>
              <ConnectWalletButton />
            </div>
          </nav>
          ...
          <section className="grid gap-12 lg:grid-cols-[1.1fr,0.9fr]">
            <div className="glass-panel rounded-3xl border border-slate-600/30 p-8 backdrop-blur-2xl">
              ...
              {isConnected ? (
                <>
                  ...
                  >
                    ...
                    <button
                      type="submit"
                      disabled={submitDisabled}
                      className="brand-gradient flex items-center justify-center rounded-2xl px-6 py-4 text-sm font-semibold uppercase tracking-[0.3em] text-slate-950 shadow-lg shadow-emerald-400/30 transition hover:shadow-emerald-200/60 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                ...
                <div className="mt-8 rounded-3xl border border-emerald-400/25 bg-slate-900/70 px-6 py-8 text-sm text-slate-200 shadow-inner shadow-slate-950">
                  <p className="text-xs uppercase tracking-[0.32em] text-emerald-200">Connect to begin</p>
                  <p className="mt-3 text-base text-slate-100">
                    Link your wallet so we can anchor ENS identity, pre-authorize USDC permits, and pin
                    Filecoin receipts before generating a booking plan.
                  </p>
                  <p className="mt-3 text-xs text-slate-400">
                    Once connected, the concierge automatically fetches calendar snapshots and surfaces
                ...
                >
                  ...
                  {entry.storage?.cid ? (
                    <p className="text-[11px] text-slate-500">
                      Filecoin CID:{" "}
                      <span className="font-mono text-slate-400">{entry.storage.cid.slice(0, 18)}…</span>
                    </p>
                  ) : null}
                </article>
              ))}
